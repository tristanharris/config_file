...
class RubyFocusedUnitTest
  ...
  def run_spec
    write_output_to_buffer("vendor/plugins/rspec/bin/spec #{current_file} -l #{line_number}")
  end
  ...
  def run_context
    ...
    if method_name
      if spec_file?
        write_output_to_buffer("vendor/plugins/rspec/bin/spec #{current_file} -l #{context_line_number}")
      else
  ...
  def run_all
    if spec_file?
      write_output_to_buffer("vendor/plugins/rspec/bin/spec #{current_file}")
    else
